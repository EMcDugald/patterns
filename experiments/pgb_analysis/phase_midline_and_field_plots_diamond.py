...
#
# No new .npz files are written; only PNG figures are saved.
#
# Supports:
#   - single input file
...
# from utils.kfield_calcs import phi_jump_mask, _central_derivs
#
#
# # ---------------------------------------------------------------------
# # small helpers
...
#
#
# def _compute_J_gradient_masked(k1, k2, x, y, mask_fd, pi_tol=np.pi / 10):
#     dx = float(x[1] - x[0])
#     dy = float(y[1] - y[0])
#
...
#     return J, mask_ok, mask_pi
#
#
# def _compute_J_safe_central(k1, k2, x, y, mask_fd, pi_tol=np.pi / 10):
#     dx = float(x[1] - x[0])
#     dy = float(y[1] - y[0])
#
#     k1 = np.asarray(k1, dtype=float)
...
#     mask_ok = mask_fd & (~mask_pi)
#
#     k1_x, k1_y = _central_derivs(k1, dx, dy, mask_ok)
#     k2_x, k2_y = _central_derivs(k2, dx, dy, mask_ok)
#
#     J = k1_x * k2_y - k1_y * k2_x
#     J = np.where(mask_ok, J, np.nan)
#     return J, mask_ok, mask_pi
...
#
#
# def _restrict_line(coord, vals, mask_1d, side=None, center_frac=0.5):
#     coord = np.asarray(coord)
#     vals = np.asarray(vals)
#     mask_1d = np.asarray(mask_1d, dtype=bool)
#
#     keep = mask_1d.copy()
#     keep &= _central_window_mask(coord, frac=center_frac)
#
#     if side == "right":
#         keep &= coord >= 0.0
#     elif side == "left":
...
#
#     J0_grad = Jf_grad = None
#     J0_safe = Jf_safe = None
#     maskJ0 = maskJf = None
#     mask_pi0 = mask_pif = None
#
#     if k10 is not None and k20 is not None:
#         J0_grad, maskJ0_grad, mask_pi0_grad = _compute_J_gradient_masked(
#             k10, k20, x, y, mask_fd, pi_tol=np.pi / 10
#         )
#         J0_safe, maskJ0_safe, mask_pi0_safe = _compute_J_safe_central(
#             k10, k20, x, y, mask_fd, pi_tol=np.pi / 10
#         )
#         maskJ0 = maskJ0_safe
#         mask_pi0 = mask_pi0_safe
#
#     if k1f is not None and k2f is not None:
#         Jf_grad, maskJf_grad, mask_pif_grad = _compute_J_gradient_masked(
#             k1f, k2f, x, y, mask_fd, pi_tol=np.pi / 10
#         )
#         Jf_safe, maskJf_safe, mask_pif_safe = _compute_J_safe_central(
#             k1f, k2f, x, y, mask_fd, pi_tol=np.pi / 10
#         )
#         maskJf = maskJf_safe
#         mask_pif = mask_pif_safe
#
#     return {
#         "path": path,
#         "x": x,
...
#
#     J0_grad = run["J0_grad"] if run["J0_grad"] is not None else np.full_like(run["u0"], np.nan, dtype=float)
#     Jf_grad = run["Jf_grad"] if run["Jf_grad"] is not None else np.full_like(run["uf"], np.nan, dtype=float)
#     J0_safe = run["J0_safe"] if run["J0_safe"] is not None else np.full_like(run["u0"], np.nan, dtype=float)
#     Jf_safe = run["Jf_safe"] if run["Jf_safe"] is not None else np.full_like(run["uf"], np.nan, dtype=float)
#
...
#         mask_pi0=mask_pi0,
#         mask_pif=mask_pif,
#         J0_gradient_masked=J0_grad,
#         Jf_gradient_masked=Jf_grad,
#         J0_safe_central=J0_safe,
#         Jf_safe_central=Jf_safe,
#         J_method_primary=np.array(
#             "safe_central", dtype=object
#         ),
#         J_gradient_masked_desc=np.array(
#             "Jacobian det(grad k) from np.gradient on k1/k2, masked by mask_fd & ~phi_jump_mask(arctan2(k2,k1), tol=pi/10).",
#             dtype=object,
#         ),
#         J_safe_central_desc=np.array(
#             "Jacobian det(grad k) from _central_derivs in utils.kfield_calcs on k1/k2, masked by mask_fd & ~phi_jump_mask(arctan2(k2,k1), tol=pi/10).",
#             dtype=object,
#         ),
#     )
#
...
#     if finite_vals:
#         allv = np.concatenate([v[np.isfinite(v)] for v in finite_vals if np.size(v) > 0])
...
#
#     jobs = [
#         ("x-midline initial", x, run["phiu0"], run["J0_safe"], run["J0_grad"], maskJ0[iy0, :], ("row", iy0)),
#         ("x-midline final",   x, run["phiuf"], run["Jf_safe"], run["Jf_grad"], maskJf[iy0, :], ("row", iy0)),
#         ("y-midline initial", y, run["phiu0"], run["J0_safe"], run["J0_grad"], maskJ0[:, ix0], ("col", ix0)),
#         ("y-midline final",   y, run["phiuf"], run["Jf_safe"], run["Jf_grad"], maskJf[:, ix0], ("col", ix0)),
#     ]
#
#     for ax, (title, coord, phi2d, Jsafe2d, Jgrad2d, mask1d, loc) in zip(axs.flat, jobs):
#         if phi2d is None or (Jsafe2d is None and Jgrad2d is None):
...
#             idx = loc[1]
#             phi1d = phi2d[idx, :]
#             Jsafe1d = None if Jsafe2d is None else Jsafe2d[idx, :]
#             Jgrad1d = None if Jgrad2d is None else Jgrad2d[idx, :]
#             xlabel = "x"
#         else:
#             idx = loc[1]
#             phi1d = phi2d[:, idx]
...
#         all_J = []
#
#         if Jsafe1d is not None:
#             _, Jsafe_line = _restrict_line(coord, Jsafe1d, mask1d, side=None, center_frac=0.5)
#             ax2.plot(c, Jsafe_line, color="C3", lw=1.6, label="J safe-central")
#             all_J.append(Jsafe_line)
#
#         if Jgrad1d is not None:
#             _, Jgrad_line = _restrict_line(coord, Jgrad1d, mask1d, side=None, center_frac=0.5)
#             ax2.plot(c, Jgrad_line, color="C0", lw=1.4, ls="--", label="J gradient")
#             all_J.append(Jgrad_line)
#
#         ax2.set_ylabel("J", color="C3")
#         ax2.tick_params(axis="y", labelcolor="C3")
#
#         if all_J:
#             J_ref = np.concatenate([np.asarray(v)[np.isfinite(v)] for v in all_J if np.size(v) > 0])
#             if J_ref.size > 0:
#                 _set_comparable_J_scale(ax, ax2, phi_line, J_ref)
#
...
#         class _Args:
#             input_file = None
#             input_dir = "//Users/edwardmcdugald/patterns/pipelines/data/sh_pgb_diamond/debug_phase/sig_pio2/raw"
#             pattern = "*.npz"
#             recursive = False
#             out_dir = "/Users/edwardmcdugald/patterns/experiments/pgb_analysis/results/phase_midline_and_field_plots_diamond/debug/sig_pio2"
#             prefer_sym = True
#             mask_tol = 0.20
#
#         a = _Args()
